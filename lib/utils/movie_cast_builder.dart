...
class MovieCastBuilder {
  Widget movieCast(String vid) {
    return FutureBuilder<QuerySnapshot>(
        future:
            videosCollection.doc(vid).collection("cast").orderBy("order").get(),
                      ...
                      return SizedBox(
                        ...
                        child: Column(
                          children: [
                            ...
                            AutoSizeText(castList[index]["name"],
                                maxLines: 2,
                                minFontSize: 10,
                                textAlign: TextAlign.center,
                                style: customTextStyleBody(
                                    fontWeight: FontWeight.bold)),
                            Text("as ${castList[index]["role"]}",
                                textAlign: TextAlign.center,
                                maxLines: 2,
        ...
        builder: (context, snapshot) {
          if (!snapshot.hasData) {
            return PlaceholderRails.instance
                .buildCastPlaceholder(context, size: 55);
          }
                    ...
                    child: MaterialButton(
                      ...
                      child: Column(
                        children: [
                          ...
                          crews[index]["isCast"]
                              ? AutoSizeText("as ${crews[index]["role"]}",
                                  textAlign: TextAlign.center,
                                  maxLines: 1,
                                  minFontSize: 8,
                                  style: customTextStyleBody(fontSize: 10))
                              : AutoSizeText("${crews[index]["position"]}",
                                  textAlign: TextAlign.center,
                                  maxLines: 1,
                                  minFontSize: 8,
                                  style: customTextStyleBody(fontSize: 10))
  ...
  Widget movieCrew(String vid) {
    return FutureBuilder<QuerySnapshot>(
        future:
            videosCollection.doc(vid).collection("crew").orderBy("order").get(),
        builder: (context, snapshot) {
          if (!snapshot.hasData) {
                      ...
                      leading: ClipRRect(
                        ...
                        child: CachedNetworkImage(
                          imageUrl: castList[index]["picture"].isEmpty
                              ...
                              : castList[index]["picture"],
                          placeholder: (context, url) =>
                              ColoredBox(color: kWhiteColor.withValues(alpha: 0.15)),
                          fit: BoxFit.cover,
                          height: 40,
                          width: 40,
                        ),
                      ),
                      title: Text(castList[index]["position"],
                          maxLines: 1,
                          style: customTextStyleBody(
                              fontSize: 12, fontWeight: FontWeight.bold)),
                      subtitle:
                          AutoSizeText(castList[index]["name"], maxLines: 2),
                    ),
                  );
                },
                separatorBuilder: (BuildContext context, int index) =>
